    ...
    try {
        ...
        Async.waterfall([
            ...
            function(next) {
                ...
                for (var thmCfgIdx in jsonFiles.thm_cfg) {
                    ...
                    for (var skinKey in thmObj.skins) {
                        skins.push(thmObj.skins[skinKey]);
                    }
                }

                // filter the exml files, skins exmls are not supported now
                var needImportExmls = [];
                for (var exmlIdx in exmlFiles) {
                    ...
                    if (needImport) {
                        needImportExmls.push(exmlPath);
                    }
                }

                if (needImportExmls.length === 0) {
                    // no any exml file need import
                    next();
                    return;
                }
            ...
            function(next) {
                ...
                // import the prefabs generated by exml files
                Editor.assetdb.import([tempResPath], AssetsRootUrl, false, function(err, results) {
                    next();
                });
            }
